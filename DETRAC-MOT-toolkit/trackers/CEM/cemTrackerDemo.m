...
savePath = 'E:\ComputerVision\Multi-target Tracking\experiment code\tracking_results\S2_L2\CEM\';

%% setup options and scene
% fill options struct
% 调效果算法时，需要改变getDCOptionsDemo的3D参数
opt=getConOptionsDemo;

% fill scene info
% 更换数据集时，需要改变getSceneInfoDCDemo的配置参数
addpath('input_trans');
...
%% load detections
[detections, nDets]=parseDetectionsFace(sceneInfo); fr=1:length(detections);
...
save([savePath 'CEM_result.mat'], 'gtInfo', 'stateInfo', 'opt');
% end