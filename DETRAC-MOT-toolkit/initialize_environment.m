...
options.imgPath = 'D:\UA-DETRAC\Secuencias de Video\'; % the path of images
options.detPath = 'D:\UA-DETRAC\DETRAC-MOT-toolkit\DETRAC-Train-Detections\'; % the path of detections
options.gtPath = 'D:\UA-DETRAC\Annotations\DETRAC-Train-Annotations-MAT\'; % the path of annotations (e.g., XML, MAT)
options.seqPath = 'D:\UA-DETRAC\DETRAC-MOT-toolkit\sequences.txt'; % the path of evaluated sequences
...
% select the type of detector, i.e., DPM, ACF, R-CNN, YOLO and CompACT. Multiple detectors are welcomed.
options.detectorSet = {'YOLO'}; 
...
options.detectionThreshold = 0:options.trackingThreStep:1; % the threshold of detection scores
...
if(strcmp(options.evaluateSeqs, 'DETRAC-Train'))
    copyfile('evaluation/seqs/trainlist-full.txt','sequences.txt'); % modify the evaluated sequences
