...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreem, char *frame)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    while(1){
        ...
        draw_detections(im, dets, nboxes, thresh, names, alphabet, l.classes, frame);
        cont++;
        free_detections(dets, nboxes);
        if(outfile){
...
void run_detector(int argc, char **argv)
{
    ...
    char *frame = argv[5];
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile, fullscreen,frame);
